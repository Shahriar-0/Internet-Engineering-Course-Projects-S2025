...
const ProfileMenu = ({ isOpen, username, onClose }) => {
    ...
    return (
        <div ref={menuRef} className={`${styles["profile-menu"]} ${isOpen ? " d-block" : ""}`}>
            ...
            <Link onClick={close} className="d-flex text-black hover-gray p-2 rounded align-middle"
                  to={AuthenticationService.isLoggedInUserAdmin() ? UrlService.urls.adminPanel : UrlService.urls.profile}>
                <img className="me-2" src={profileIcon} alt="profile-icon" />  Profile
            </Link>
            <Link onClick={close} className="d-flex text-black hover-gray p-2 rounded align-middle"
                  to={AuthenticationService.isLoggedInUserAdmin() ? UrlService.urls.adminPanel : UrlService.urls.myBooks}>
