    ...
    const headerContent = (
        <header className="shadow py-3 px-5 sticky-top bg-custom-white">
            <div
                ...
                <MioBookLogo/>
                <HeaderSearchBox/>
                <ProfileIcon username={AuthenticationService.getCurrentUser()?.username} onClick={() => setIsOpen(!isOpen)}/>
                <ProfileMenu onClose={() => setIsOpen(false)} isOpen={isOpen} username={AuthenticationService.getCurrentUser()?.username}/>
            </div>
        </header>
    )
